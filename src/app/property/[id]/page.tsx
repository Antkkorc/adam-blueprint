...
export default async function PropertyDetailPage({ params }: PageProps) {
  ...
  const whatsappMessage = encodeURIComponent(
    ...
    } (Plot ${property.plot_number || "N/A"}). Please send me more details.`
  );

  // Pinned location: coordinates if we have them, otherwise the address text
  const hasCoords = property.latitude != null && property.longitude != null;
  const mapQuery = hasCoords
    ? `${property.latitude},${property.longitude}`
    : `${property.location || property.city || "Gaborone"}, Botswana`;
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                {property.featured && (
                  <span className="px-2.5 py-1 rounded-full bg-cyan-400 text-slate-950 text-[10px] font-extrabold uppercase">
                    Featured
                  </span>
                )}
                {property.verified && (
                  <span className="px-2.5 py-1 rounded-full bg-emerald-500/15 border border-emerald-500/40 text-emerald-400 text-[10px] font-extrabold uppercase">
                    Verified
                  </span>
                )}
                {property.title_deed && (
                  <span className="px-2.5 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-300 text-[10px] font-extrabold uppercase">
                    Title Deed
                  </span>
                )}
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {stats.map((stat) => (
                <div
                  key={stat.label}
                  className="bg-slate-900 border border-slate-800 rounded-xl p-3 space-y-1"
                >
            ...
            <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-3">
              ...
              <p className="text-slate-300 text-sm leading-relaxed whitespace-pre-line">
                {property.overview || property.description || "No overview provided."}
              </p>
              ...
              <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {amenities.map((amenity) => (
                    <span
                      key={amenity}
                      className="text-xs px-3 py-1.5 rounded-full bg-slate-950 border border-slate-800 text-slate-300"
                    >
                      {amenity}
                    </span>
                  ))}
            ...
            {sketchPlan.length > 0 && (
              <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
                <h2 className="text-lg font-bold flex items-center gap-2">
                  <DraftingCompass className="w-5 h-5 text-cyan-400" />
                  Sketch Plan / Floor Plan
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {sketchPlan.map((img, i) => (
                    <img
                      ...
                      src={img}
                      alt={`Sketch plan ${i + 1}`}
                      className="w-full h-64 object-contain bg-white rounded-xl border border-slate-800 p-2"
                    />
                  ))}
                </div>
              </section>
            ...
            <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
              ...
              <p className="text-slate-400 text-xs">
                {hasCoords
                  ? `Pinned at ${property.latitude}, ${property.longitude}`
                  : `Approximate area: ${mapQuery}`}
              </p>
            </section>
          </div>

          {/* ============ RIGHT: AGENT / OWNER CARD ============ */}
          <div className="space-y-4 lg:sticky lg:top-24 self-start">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-xs">
                    {property.intent === "rent" ? "Rental Agent" : "Property Agent / Owner"}
                  </p>
                </div>
              </div>
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-full py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 text-xs font-bold flex items-center justify-center gap-2 transition-colors"
              >
              ...
              <a
                ...
                className="w-full py-3 rounded-xl border border-slate-700 bg-slate-950 hover:border-cyan-400 text-white text-xs font-bold flex items-center justify-center gap-2 transition-colors"
              >
              ...
              <div className="relative">
                <SavePropertyButtonWrapper saved={saved} propertyId={property.id} />
              </div>
            </div>
...
// Small wrapper so the save button fills the card width
function SavePropertyButtonWrapper({
  saved,
  propertyId,
}: {
  saved: boolean;
  ...
  return (
    <div className="[&>button]:w-full [&>button]:justify-center [&>button]:flex [&>button]:items-center [&>button]:gap-2 [&>button]:px-5 [&>button]:py-3 [&>button]:rounded-xl [&>button]:border [&>button]:text-xs [&>button]:font-bold [&>button]:static">
      <SavePropertyButton propertyId={propertyId} initialSaved={saved} size="lg" />
    </div>
  );
}