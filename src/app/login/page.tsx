...
export default function AuthPage() {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    if (tab === "login") {
      ...
      // FIXED: redirectTo changed to emailRedirectTo for signUp
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12 relative overflow-hidden" suppressHydrationWarning>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative z-10 w-full max-w-md">
        <div className="glass rounded-3xl p-8 shadow-2xl space-y-6" suppressHydrationWarning>
          ...
          {error && <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 text-xs text-center">{error}</div>}
          {message && <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs text-center">{message}</div>}

          {tab !== "forgot" && tab !== "phone" && (
            <div className="flex p-1 rounded-2xl bg-white/5 border border-white/5">
              {(["login", "register"] as AuthTab[]).map((t) => (
                <button key={t} type="button" onClick={() => handleTabChange(t)} className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all btn-pop ${tab === t ? "bg-cyan-400 text-slate-950" : "text-slate-400 hover:text-white"}`}>
          ...
          <AnimatePresence mode="wait">
            {tab === "login" || tab === "register" ? (
              <motion.div key="auth" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                <button type="button" onClick={() => handleOAuth("google")} className="w-full py-3 rounded-xl bg-white/5 border border-white/10 hover:border-white/20 text-white text-xs font-semibold flex items-center justify-center gap-2 transition-all btn-pop">
                ...
                <button type="button" onClick={() => handleOAuth("facebook")} className="w-full py-3 rounded-xl bg-[#1877F2]/10 border border-[#1877F2]/30 text-[#1877F2] text-xs font-semibold flex items-center justify-center gap-2 transition-all btn-pop hover:bg-[#1877F2]/20">
                ...
                <form onSubmit={handleEmailAuth} className="space-y-3">
                  <div className="relative group">
                    ...
                    <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email address" className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-sm text-white placeholder:text-slate-600 outline-none focus:border-cyan-500/50 transition-all" />
                  </div>
                  <div className="relative group">
                    <Lock className="absolute left-3 top-3 w-4 h-4 text-slate-500 group-focus-within:text-cyan-400 transition-colors" />
                    <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-sm text-white placeholder:text-slate-600 outline-none focus:border-cyan-500/50 transition-all" />
                  </div>
                  {tab === "login" && (
                    <button type="button" onClick={() => handleTabChange("forgot")} className="text-[11px] text-cyan-400 hover:underline w-full text-right">Forgot password?</button>
                  )}
                  <button type="submit" disabled={loading} className="w-full py-3 rounded-xl bg-cyan-400 hover:bg-cyan-300 text-slate-950 font-bold text-xs transition-all btn-pop neon-glow-hover flex items-center justify-center gap-2 disabled:opacity-50">
                ...
                {phoneStep === "input" ? (
                  <form onSubmit={handleSendPhoneOtp} className="space-y-4">
                    ...
                    <div className="relative">
                      ...
                      <input type="tel" required value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+267 71 000 000" className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-sm text-white placeholder:text-slate-600 outline-none focus:border-cyan-500/50" />
                    </div>
                  ...
                  <form onSubmit={handleVerifyOtp} className="space-y-4">
                    ...
                    <div className="flex gap-2 justify-center">
                      {otp.map((digit, i) => (
                        <input key={i} id={`otp-${i}`} type="text" inputMode="numeric" maxLength={1} value={digit} onChange={(e) => handleOtpChange(i, e.target.value)} className="w-10 h-10 text-center bg-white/5 border border-white/10 rounded-xl text-base font-bold text-white outline-none focus:border-cyan-400 transition-all" style={{ boxShadow: digit ? "0 0 15px rgba(34,211,238,0.2)" : "none" }} />
                      ))}
                    </div>
                    <button type="submit" disabled={loading} className="w-full py-3 rounded-xl bg-cyan-400 text-slate-950 font-bold text-xs transition-all btn-pop neon-glow-hover flex items-center justify-center gap-2">
                      {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Verify & Login"}
                    </button>
                    <button type="button" onClick={() => setPhoneStep("input")} className="w-full text-[11px] text-cyan-400 hover:underline">Didn&apos;t receive code? Resend</button>
                  </form>
                )}
              </motion.div>
            ) : null}
