...
export default function AdminPropertySubmissionReview({ submission }: { submission: Submission }) {
  ...
  return <article className="rounded-2xl border border-slate-800 bg-slate-900 p-5 space-y-3">
    ...
    {Array.isArray(submission.images) && <div className="flex flex-wrap gap-3">{submission.images.map((url: string, i: number) => <a className="group" key={url} href={url} target="_blank" rel="noreferrer"><img src={url} alt={`Submitted property photo ${i + 1}`} className="h-24 w-32 rounded-lg object-cover ring-1 ring-slate-700 group-hover:ring-cyan-400" /><span className="text-xs text-cyan-400 underline">Photo {i + 1}</span></a>)}</div>}
